...
void event1(Backpack &backpack, Stats &stats, std::string time_of_day) {
    ...
    if (response == 1) {
        ...
        if (response == 1) {
            std::cout << "items.find(item_name)->second and look around. There is dense forest in every direction. You head west once again, brushing branches and foliage out of your way as you go along. \n";
            pause();
